...
function heaterHelper() {
	var currCostPerYear = calcTotalCost(state, heaterYear, sqFeet);
	var heaterUnitStr = document.getElementById('heaterUnitText').innerHTML;
	heaterUnitStr = heaterUnitStr.replace("[manufacturer]", heaterManf);
	heaterUnitStr = heaterUnitStr.replace("[year]", heaterYear);
	...
	if(bestMatchPrices[0] != currTotalCost) {
		...
		var yearsReq = (price[bestMatches[0]]/(currCostPerYear - calcTotalCostSEER(state, seer[bestMatches[0]], sqFeet))).toFixed(2);
		var carbonSaved = (calcTotalCarbonYear(state, heaterYear, sqFeet) - calcTotalCarbonSEER(state, seer[bestMatches[0]], sqFeet)).toFixed(0);
		var carbonPercent = ((1 - calcTotalCarbonSEER(state, seer[bestMatches[0]], sqFeet)/calcTotalCarbonYear(state, heaterYear, sqFeet))*100).toFixed(0);
